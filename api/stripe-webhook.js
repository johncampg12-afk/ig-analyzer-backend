...
module.exports = async (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    const amountTotal = session.amount_total / 100; // Convertir a euros
    ...
    console.log(`✅ Pago recibido de ${email} - ${amountTotal}€`);
    ...
    console.log(`✅ Licencia creada: ${licenseKey} para ${email}`);

    // ENVIAR EMAIL CON RESEND
    try {
      const { data: emailData, error: emailError } = await resend.emails.send({
        from: 'IG Analyzer PRO <onboarding@resend.dev>', // Cambia esto cuando tengas dominio verificado
        to: [email],
      ...
      if (emailError) {
        ...
        console.log('✅ Email enviado a:', email, 'ID:', emailData.id);
      }
    } catch (emailErr) {
